...
(* canonical names: *)
#[global] Instance Z_equiv: Equiv Z := eq.
#[global] Instance Z_plus: Plus Z := Zplus.
#[global] Instance Z_0: Zero Z := 0%Z.
#[global] Instance Z_1: One Z := 1%Z.
#[global] Instance Z_mult: Mult Z := Zmult.
#[global] Instance Z_negate: Negate Z := Z.opp.
...
(* misc: *)
#[global] Instance: ∀ x y : Z, Decision (x = y) := Z.eq_dec.

Add Ring Z: (rings.stdlib_ring_theory Z).

(* * Embedding N into Z *)
#[global] Instance inject_N_Z: Cast N Z := Z_of_N.

#[global] Instance: SemiRing_Morphism Z_of_N.
Proof.
...
#[global] Instance: Injective Z_of_N.
Proof.
  repeat (split; try apply _).
...
#[global] Instance: Proper (=) Npair_to_Z.
Proof.
...
#[global] Instance: SemiRing_Morphism Npair_to_Z.
Proof.
  repeat (split; try apply _).
   intros [xp xn] [yp yn].
...
#[global] Instance: Injective Npair_to_Z.
Proof.
  split; try apply _.
...
#[global] Instance Z_to_Npair: Inverse Npair_to_Z := λ x,
  match x with
  | Z0 => C 0 0
  | Zpos p => C (Npos p) 0
  | Zneg p => C 0 (Npos p)
...
#[global] Instance: Surjective Npair_to_Z.
Proof. split; try apply _. intros [|?|?] ? E; now rewrite <-E. Qed. 

#[global] Instance: Bijective Npair_to_Z := {}.

#[global] Instance: SemiRing_Morphism Z_to_Npair.
Proof. change (SemiRing_Morphism (Npair_to_Z⁻¹)). split; apply _. Qed.

#[global] Instance: IntegersToRing Z := integers.retract_is_int_to_ring Npair_to_Z.
#[global] Instance: Integers Z := integers.retract_is_int Npair_to_Z.

#[global] Instance Z_le: Le Z := Z.le.
#[global] Instance Z_lt: Lt Z := Z.lt.

#[global] Instance: SemiRingOrder Z_le.
Proof.
...
#[global] Instance: TotalRelation Z_le.
Proof.
  intros x y.
...
#[global] Instance: FullPseudoSemiRingOrder Z_le Z_lt.
Proof.
...
#[global] Instance inject_nat_Z: Cast nat Z := Z_of_nat.

#[global] Instance: SemiRing_Morphism Z_of_nat.
Proof.
  repeat (split; try apply _).
   exact Znat.inj_plus.
  exact Znat.inj_mult.
...
(* absolute value *)
#[global] Program Instance Z_abs_nat: IntAbs Z nat := λ x,
  match x with
  | Z0 => inl (0:nat)
...
#[global] Program Instance Z_abs_N: IntAbs Z N := λ x,
  match x with
  | Z0 => inl (0:N)
  | Zpos p => inl (Npos p)
  | Zneg p => inr (Npos p)
...
(* Efficient nat_pow *)
#[global] Program Instance Z_pow: Pow Z (Z⁺) := Z.pow.

#[global] Instance: NatPowSpec Z (Z⁺) Z_pow.
Proof.
...
#[global] Instance Z_Npow: Pow Z N := λ x n, Z.pow x ('n).

#[global] Instance: NatPowSpec Z N Z_Npow.
Proof.
...
(* Efficient shiftl *)
#[global] Program Instance Z_shiftl: ShiftL Z (Z⁺) := Z.shiftl.

#[global] Instance: ShiftLSpec Z (Z⁺) Z_shiftl.
Proof.
...
#[global] Instance Z_Nshiftl: ShiftL Z N := λ x n, Z.shiftl x ('n).

#[global] Instance: ShiftLSpec Z N Z_Nshiftl.
Proof.
  apply shiftl_spec_from_nat_pow.
  intros x n.
  apply Z.shiftl_mul_pow2.
...
#[global] Program Instance Z_abs: Abs Z := Z.abs.
Next Obligation.
  split; intros E.
   now apply Z.abs_eq.
  now apply Z.abs_neq.
...
#[global] Instance Z_div: DivEuclid Z := Z.div.
#[global] Instance Z_mod: ModEuclid Z := Zmod.

#[global] Instance: EuclidSpec Z _ _.
Proof.
  split; try apply _.
     exact Z.div_mod.
