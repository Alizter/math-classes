...
#[global] Instance Q_eq: Equiv Q := Qeq.
#[global] Instance Q_0 : Zero Q := 0%Q.
#[global] Instance Q_1 : One Q := 1%Q.
#[global] Instance Q_opp : Negate Q := Qopp.
#[global] Instance Q_plus : Plus Q := Qplus.
#[global] Instance Q_mult : Mult Q := Qmult.
#[global] Instance Q_recip : DecRecip Q := Qinv.

(* properties: *)
#[global] Instance: Setoid Q := {}.

#[global] Instance: DecField Q.
Proof dec_fields.from_stdlib_field_theory Qfield.Qsft eq_refl.

(* misc: *)
#[global] Instance: ∀ x y: Q, Decision (x = y) := Qeq_dec.

#[global] Instance inject_Z_Q: Cast Z Q := inject_Z.

#[global] Instance: Proper ((=) ==> (=)) inject_Z.
Proof. intros x y H. unfold inject_Z. repeat red. simpl. now rewrite H. Qed.

#[global] Instance: SemiRing_Morphism inject_Z.
Proof.
  repeat (split; try apply _).
  intros x y. repeat red. simpl. now rewrite ?Zmult_1_r.
Qed.

#[global] Instance: Injective inject_Z.
Proof.
 constructor. 2: apply _.
 intros x y. change (x * 1 = y * 1 → x = y). rewrite 2!rings.mult_1_r. intuition.
Qed.

#[global] Program Instance Q_to_fracZ: Cast Q (Frac Z) := λ x, frac (Qnum x) (Zpos (Qden x)) _.

#[global] Instance: Proper ((=) ==> (=)) Q_to_fracZ.
Proof. intros ? ? E. easy. Qed.

#[global] Instance: SemiRing_Morphism Q_to_fracZ.
Proof. repeat (split; try apply _). Qed.

#[global] Instance: Injective Q_to_fracZ.
Proof. split; try apply _. intros ? ? E. easy. Qed.

#[global] Instance: RationalsToFrac Q := rationals.alt_to_frac Q_to_fracZ.
#[global] Instance: Rationals Q := rationals.alt_Build_Rationals Q_to_fracZ inject_Z.

(* order: *)
#[global] Instance Q_le: Le Q := Qle.
#[global] Instance Q_lt: Lt Q := Qlt.

#[global] Instance: SemiRingOrder Q_le.
Proof.
...
#[global] Instance: TotalRelation Q_le.
Proof.
  intros x y.
...
#[global] Instance: FullPseudoSemiRingOrder Q_le Q_lt.
Proof.
...
#[global] Program Instance: ∀ x y: Q, Decision (x ≤ y) := λ y x,
...
#[global] Program Instance: Abs Q := Qabs.
Next Obligation.
  split; intros E.
   now apply Qabs_pos.
  now apply Qabs_neg.
...
#[global] Instance Q_pow: Pow Q Z := Qpower.

#[global] Instance: IntPowSpec Q Z Q_pow.
Proof.
  split.
     apply _.
    reflexivity.
...
#[global] Instance Q_Npow: Pow Q N := λ x n, Qpower x (cast N Z n).

#[global] Instance: NatPowSpec Q N Q_Npow.
Proof.
  split.
...
#[global] Instance Q_shiftl: ShiftL Q Z := λ x k,
  match k with
  | Z0 => x
...
#[global] Instance: ShiftLSpec Q Z Q_shiftl.
Proof.
